...
# Функция для парсинга порта из файла settings.py
get_gradio_port() {
    local file_path="wg_qr_generator/settings.py"
    local port

    # Извлекаем значение GRADIO_PORT
    port=$(grep -oP 'GRADIO_PORT\s*=\s*\K\d+' "$file_path")

    # Проверяем, найден ли порт
    if [[ -n "$port" ]]; then
        echo "Найденный порт: $port"
        # Здесь вы можете вставить код для использования переменной $port
    else
        echo "Порт не найден."
    fi
}
